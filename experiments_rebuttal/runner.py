...
def fixed_split_plan(dataset, variants, seeds=(0, 1, 2, 3), with_baseline=True):
    jobs = []
    if with_baseline:
        jobs += [dict(dataset=dataset, model="baseline", seed=sd, fold=1, epochs=None,
                      variant="fast", overrides=dict(FAST)) for sd in seeds]
    V = dict(IMPROVE_VARIANTS)
    jobs += [dict(dataset=dataset, model="topo", seed=sd, fold=1, epochs=None,
                  variant=vn + "_fast", overrides=dict(V[vn], **FAST))
             for vn in variants for sd in seeds]
    return jobs


def zinc_plan(variants, **kw):
    return fixed_split_plan("ZINC", variants, **kw)
