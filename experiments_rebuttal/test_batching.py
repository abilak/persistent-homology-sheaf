...
def test_batch_vs_single(node_level, multiplicity, B=6, M=12, d=8):
...
def test_equivariance(node_level, multiplicity, M=11, d=6, trials=3):
    worst = 0.0
...
if __name__ == '__main__':
    ...
    for nl in (True, False):
        for mult in (False, True):
            df, dg = test_batch_vs_single(nl, mult)
            good = df < 1e-5 and dg < 1e-5
            ok &= good
            print(f"  node_level={nl!s:<5} multiplicity={mult!s:<5} "
