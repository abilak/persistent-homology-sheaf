...
LEARNING_RATES = {'COLLAB': 0.0001, 'IMDBBINARY': 0.00005, 'IMDBMULTI': 0.0001, 'MUTAG': 0.0005, 'NCI1':0.0001, 'NCI109':0.0001, 'PROTEINS': 0.0005, 'PTC': 0.0001, 'DD': 0.0001}
DECAY_RATES = {'COLLAB': 0.5, 'IMDBBINARY': 0.5, 'IMDBMULTI': 0.75, 'MUTAG': 0.5, 'NCI1':0.75, 'NCI109':0.75, 'PROTEINS': 0.5, 'PTC': 1.0, 'DD': 0.5}
CHOSEN_EPOCH = {'COLLAB': 150, 'IMDBBINARY': 100, 'IMDBMULTI': 150, 'MUTAG': 150, 'NCI1': 200, 'NCI109': 200, 'PROTEINS': 100, 'PTC': 180, 'DD': 150}
...
def process_config(json_file, dataset_name, use_topology=None):
    ...
    if config.exp_name == "10fold_cross_validation":
        ...
        # Allow config-level overrides for learning_rate and decay_rate
        if not hasattr(config.hyperparams, 'learning_rate'):
            config.hyperparams.learning_rate = LEARNING_RATES[config.dataset_name]
        if not hasattr(config.hyperparams, 'decay_rate'):
